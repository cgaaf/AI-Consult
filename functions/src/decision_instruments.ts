export function centorCalc(
    age: number, 
    tonsilarExudate: string, 
    cervicalLymph: string, 
    fever: string, 
    cough: string): string {
    ...
    // Calculate the score
        ...
        // Check tonsil criteria
            if(tonsilarExudate === "true") { score += 1}

        // Check lymph node criteria
            if(cervicalLymph === "true") { score += 1 }
        
        // Check fever crtieria
            if(fever === "true") { score += 1 }
...
export function heartCalc(
    ...
    switch (suspicion) {
        case "slightly suspicious": score += 0;
            break;
        case "moderately suspicious": score += 1;
            break;
        case "highly suspicious": score += 2;
            break;
        default: break;
    }
    ...
    switch (ekg) {
        case "normal": score += 0;
            break;
        case "non-specific repolarization disturbance": score += 1;
            break;
        case "significant st depression": score += 2;
            break;
        default: break;
    }
    ...
    if (historyOfCAD === 'true') { score += 2 }
    else if (numRiskFactors < 1) { score += 0 }
    else if (numRiskFactors < 3) { score += 1 }
    else { score += 2 }
    ...
    switch (initialTrop) {
        case "normal": score += 0;
            break;
        case "1-3x normal": score += 1;
            break;
        case "beyond 3x normal": score += 2;
            break;
    }
